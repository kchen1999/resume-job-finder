...
async def parse_job_posting(markdown, count):
    try:
        ...
        logger.exception("Error calling Groq API (refine_data):", e)
...
async def infer_work_model(job_text):
    try:
        ...
        logger.exception(f"Error calling Groq API (work_model): {e}")
        with sentry_sdk.push_scope() as scope:
...
async def infer_experience_level(job_title, job_text):
    try:
        ...
        logger.exception(f"Error calling Groq API (experience_level): {e}")
        with sentry_sdk.push_scope() as scope:
