...
from utils import extract_total_job_count, extract_job_urls, parse_job_data_from_markdown, is_job_within_date_range, get_relative_posted_time, flatten_field, extract_job_metadata_fields, pause_briefly, infer_experience_level_from_title, normalize_experience_level
from utils import extract_job_links, process_markdown_to_job_links, parse_json_block_from_text, clean_string, get_posted_date, enrich_job_data, extract_posted_date_by_class, extract_logo_src, override_experience_level_with_title, set_default_work_model
...
@patch("utils.random.uniform", return_value=0.3)
...
@pytest.mark.asyncio
@patch("utils.asyncio.sleep", new_callable=AsyncMock)
@patch("utils.random.uniform", return_value=1.5)
...
@pytest.mark.asyncio
@patch("utils.random.random", return_value=0.9)
...
@pytest.mark.asyncio
@patch("utils.get_posted_date", return_value="05/05/2024")  
@patch("utils.random.random", return_value=0.9)   
...
@pytest.mark.asyncio
@patch("utils.get_posted_date", return_value="04/04/2024")  
@patch("utils.random.random", return_value=0.9)   
...
@pytest.mark.asyncio
@patch("utils.get_posted_date", return_value="03/03/2024")  
@patch("utils.random.random", return_value=0.9)   
...
@patch("utils.extract_job_links", return_value=[])
...
@pytest.mark.asyncio
@patch("utils.parse_job_posting", new_callable=AsyncMock)
@patch("utils.parse_json_block_from_text")
async def test_parse_job_data_from_markdown(mock_extract_json, mock_extract_fields):
    ...
    mock_extract_json.return_value = job_data
    result = await parse_job_data_from_markdown(job_md, count)
    assert result == job_data

@pytest.mark.asyncio
@patch("utils.repair_json")
@patch("utils.clean_string")
@patch("utils.parse_job_posting")
async def test_parse_job_data_from_markdown_with_messy_string_json(mock_extract_fields, mock_clean_string, mock_repair_json):
    job_md = "mock job markdown"
    count = 1
...
def test_is_job_within_date_range_valid_dates(days_ago, within_days, expected):
    ...
    assert is_job_within_date_range(job_data, within_days=within_days) is expected
...
@patch("utils.get_relative_posted_time")
def test_enrich_job_data(mock_get_relative_posted_time):
    job_data = {
