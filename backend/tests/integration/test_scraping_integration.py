...
def assert_valid_job(job):
    ...
    for field in ['logo_link', 'location', 'location_search', 'experience_level', 'salary', 'description', 'quick_apply_url', 'job_url']:
        val = job.get(field)
...
async def test_scrape_and_send_jobs():
    ...
    async def mock_scrape_job_listing(base_url, location_search, pagesize=20):
        result = await scrape_job_listing(base_url, location_search, pagesize)
        scrape_results.update(result)
        return result

    with patch("scraper.validate_and_db_insert.send_page_jobs_to_node", new=mock_send_page_jobs_to_node), \
